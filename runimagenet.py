...
saved_data = torch.load("./dataset/imagenet-data/data_sample100_resolution256.pt")
saved_label = torch.load("./dataset/imagenet-data/label_sample100_resolution256.pt")
...
class CustomImageNet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return image, label
dataset = CustomImageNet(saved_data=saved_data, saved_label=saved_label)
...
exp_name = f'imagenet_num_epochs_{num_epochs}_size_batch_{size_batch}_num_exps_{num_exps}'
...
config['lr']=attack['lr']
my_trainer = trainer(config,attack,model,dataset)

reconstructed_image,avg_score = my_trainer.attack_training()
ssim_list = []
fsim_list = []
psnr_list = []
lpips_list = []
...
for num_exp in range(config['num_exps']):
    result_folder_name = f'./result_imagenet/{exp_name}/batch-{num_exp}'
