...
def calculate_iqa(dataloader,reconstruct_data,config):
    ...
    for metric in ['ssim','fsim','psnr','lpips']:
        similar_pairs = []
        each_score= 0
        ...
        for recon_idx in range(len(reconstruct_data[-1])):      # Loop each reconstructed image 
            ...
            for ori_idx in range(len(dataloader)):              # Find the ground truth images that are similar with the reconstructed images['recon_idx'] (max SSIM and FSIM)
                if (metric == 'ssim'):
                    ...
                    each_score = psnr(torch.clamp(torch.stack([dataloader[ori_idx]]).cpu(),min=0,max=1),torch.clamp(torch.stack([reconstruct_data[-1][recon_idx]]),min=0,max=1),data_range=1).cpu().item()
                elif (metric == 'lpips'):
                    lpips = LPIPS()
                    each_score = lpips(torch.clamp(torch.stack([dataloader[ori_idx]]).cpu(),min=0,max=1),torch.clamp(torch.stack([reconstruct_data[-1][recon_idx]]),min=0,max=1)).cpu().item()
                if (each_score > max_score) and (metric != 'lpips'):                    # Found the better similar image
                    max_score = each_score
                    max_idx = ori_idx
                elif (-each_score > max_score) and (metric == 'lpips'):
                    max_score = -each_score
                    max_idx = ori_idx
        ...
        for attack_iter in range(len(reconstruct_data)):                                            # Loop each attack iteration
            ...
            for reconstrcuted_idx, ground_truth_idx in similar_pairs:                               # Loop similar pairs
                ...
                if (metric == 'ssim'):
                    ...
                    each_score = psnr(torch.clamp(torch.stack([dataloader[ground_truth_idx]]),min=0,max=1),torch.clamp(torch.stack([reconstruct_data[attack_iter][reconstrcuted_idx]]),min=0,max=1),data_range=1).cpu().item()
                elif (metric == 'lpips'):
                    lpips = LPIPS()
                    each_score = lpips(torch.clamp(torch.stack([dataloader[ground_truth_idx]]),min=0,max=1),torch.clamp(torch.stack([reconstruct_data[attack_iter][reconstrcuted_idx]]),min=0,max=1)).cpu().item()
